...
@section('title', 'PHILHEALTH CONTRIBUTION REPORT')

@section('breadcrumbs')
    <span>REPORTS</span> / <span>PAYROLL REPORTS</span> / <span class="highlight">PHILHEALTH CONTRIBUTION REPORT</span>
@endsection
...
<script>
    ...
    $(document).on('click', '.js-phic-employee', function (e) {
        e.preventDefault();
        var employeeId = $(this).data('employee-id');
        var employeeName = $(this).data('employee-name') || 'Employee';

        $('#phicBreakdownEmployee').text(employeeName);
        $('#phicBreakdownBody').html('<tr><td colspan="4" class="text-center">Loading...</td></tr>');
        $('#phicTotalEe').text('0.00');
        $('#phicTotalEr').text('0.00');
        $('#phicTotalContribution').text('0.00');
        $('#phicBreakdownModal').modal('show');

        $.get('/reports/payroll/philhealth-contribution/breakdown/' + employeeId, function (resp) {
            ...
            $('#phicBreakdownBody').html(html);
            $('#phicTotalEe').text(formatCurrency(resp && resp.totals ? resp.totals.ee : 0));
            $('#phicTotalEr').text(formatCurrency(resp && resp.totals ? resp.totals.er : 0));
            $('#phicTotalContribution').text(formatCurrency(resp && resp.totals ? resp.totals.total : 0));
        }).fail(function () {
            $('#phicBreakdownBody').html('<tr><td colspan="4" class="text-center text-danger">Unable to load breakdown.</td></tr>');
        });
    });
</script>
@endsection
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center no-print">
                <h5 class="card-title mb-0">PHILHEALTH CONTRIBUTION REPORT</h5>
                <div>
                    <button type="button" class="btn btn-sm btn-light mr-2" onclick="printMainContributionTable('.print-table-area table', 'PHILHEALTH CONTRIBUTION REPORT')">Print</button>
                ...
                <form method="GET" class="mb-2 not no-print">
                    <div class="form-row align-items-end">
                        ...
                        <div class="col-md-4 d-flex">
                            ...
                            <a href="/reports/payroll/philhealth-contribution" class="btn btn-sm btn-light">Reset</a>
                        </div>
                    </div>
                </form>
                <div class="table-responsive print-table-area">
                    <table class="table table-bordered table-sm mb-0">
                        <thead>
                            <tr>
                                <th>Employee Name</th>
                                <th class="text-right">Total PHILHEALTH Contribution</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($rows as $row)
                            ...
                            @empty
                                <tr>
                                    <td colspan="2" class="text-center">No Philhealth contribution records found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
...
<div class="modal fade" id="phicBreakdownModal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">PHILHEALTH Contribution Breakdown: <span id="phicBreakdownEmployee">Employee</span></h5>
            ...
            <div class="modal-body">
                <div class="table-responsive">
                    <table class="table table-bordered table-sm mb-0">
                        ...
                        <tbody id="phicBreakdownBody">
                            <tr><td colspan="4" class="text-center">No contribution breakdown found.</td></tr>
                        </tbody>
                    </table>
                </div>
                <div class="mt-3 text-right">
                    <div><strong>Total EE:</strong> <span id="phicTotalEe">0.00</span></div>
                    <div><strong>Total ER:</strong> <span id="phicTotalEr">0.00</span></div>
                    <div><strong>Grand Total:</strong> <span id="phicTotalContribution">0.00</span></div>
                </div>
                <div class="mt-3 d-flex justify-content-start no-print">
                    <button type="button" class="btn btn-sm btn-light" onclick="printContributionModal('#phicBreakdownModal', '#phicBreakdownEmployee')">Print</button>
                </div>
            </div>
        </div>
    </div>
