...
@section('title', 'PAG-IBIG CONTRIBUTION REPORT')

@section('breadcrumbs')
    <span>REPORTS</span> / <span>PAYROLL REPORTS</span> / <span class="highlight">PAG-IBIG CONTRIBUTION REPORT</span>
@endsection
...
<style>
    ...
    @media print {
        ...
        .print-table-area,
        .print-table-area * { visibility: visible !important; }
        .print-table-area {
            position: absolute;
            left: 0;
            top: 0;
...
<script>
    ...
    $(document).on('click', '.js-pagibig-employee', function (e) {
        e.preventDefault();
        var employeeId = $(this).data('employee-id');
        var employeeName = $(this).data('employee-name') || 'Employee';

        $('#pagibigBreakdownEmployee').text(employeeName);
        $('#pagibigBreakdownBody').html('<tr><td colspan="4" class="text-center">Loading...</td></tr>');
        $('#pagibigTotalEe').text('0.00');
        $('#pagibigTotalEr').text('0.00');
        $('#pagibigTotalContribution').text('0.00');
        $('#pagibigBreakdownModal').modal('show');

        $.get('/reports/payroll/pagibig-contribution/breakdown/' + employeeId, function (resp) {
            ...
            $('#pagibigBreakdownBody').html(html);
            $('#pagibigTotalEe').text(formatCurrency(resp && resp.totals ? resp.totals.ee : 0));
            $('#pagibigTotalEr').text(formatCurrency(resp && resp.totals ? resp.totals.er : 0));
            $('#pagibigTotalContribution').text(formatCurrency(resp && resp.totals ? resp.totals.total : 0));
        }).fail(function () {
            $('#pagibigBreakdownBody').html('<tr><td colspan="4" class="text-center text-danger">Unable to load breakdown.</td></tr>');
        });
    });
</script>
@endsection
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center no-print">
                <h5 class="card-title mb-0">PAG-IBIG CONTRIBUTION REPORT</h5>
                <div>
                    <button type="button" class="btn btn-sm btn-light mr-2" onclick="printMainContributionTable('.print-table-area table', 'PAG-IBIG CONTRIBUTION REPORT')">Print</button>
            ...
            <div class="card-body">
                <form method="GET" class="mb-2 not no-print">
                    <div class="form-row align-items-end">
                        ...
                        <div class="col-md-4 d-flex">
                            ...
                            <a href="/reports/payroll/pagibig-contribution" class="btn btn-sm btn-light">Reset</a>
                        </div>
                    </div>
                </form>
                <div class="table-responsive print-table-area">
                    <table class="table table-bordered table-sm mb-0">
                        <thead>
                            <tr>
                                <th>Employee Name</th>
                                <th class="text-right">Total PAG-IBIG Contribution</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($rows as $row)
                                ...
                                <tr>
                                    <td>
                                        <a href="#" class="js-pagibig-employee"
                            ...
                            @empty
                                <tr>
                                    <td colspan="2" class="text-center">No PAG-IBIG contribution records found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div class="d-flex justify-content-between align-items-center mt-2 no-print">
                    ...
                    <div>
                        <small class="mr-2">Showing {{ $rows->firstItem() ?? 0 }} to {{ $rows->lastItem() ?? 0 }} of {{ $rows->total() }} entries</small>
                        @if($rows->onFirstPage())
...
<div class="modal fade" id="pagibigBreakdownModal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">PAG-IBIG Contribution Breakdown: <span id="pagibigBreakdownEmployee">Employee</span></h5>
            ...
            <div class="modal-body">
                <div class="table-responsive">
                    <table class="table table-bordered table-sm mb-0">
                        ...
                        <tbody id="pagibigBreakdownBody">
                            <tr><td colspan="4" class="text-center">No contribution breakdown found.</td></tr>
                        </tbody>
                    </table>
                </div>
                <div class="mt-3 text-right">
                    <div><strong>Total EE:</strong> <span id="pagibigTotalEe">0.00</span></div>
                    <div><strong>Total ER:</strong> <span id="pagibigTotalEr">0.00</span></div>
                    <div><strong>Grand Total:</strong> <span id="pagibigTotalContribution">0.00</span></div>
                </div>
                <div class="mt-3 d-flex justify-content-start no-print">
                    <button type="button" class="btn btn-sm btn-light" onclick="printContributionModal('#pagibigBreakdownModal', '#pagibigBreakdownEmployee')">Print</button>
                </div>
            </div>
        </div>
    </div>
